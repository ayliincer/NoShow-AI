...
def degerlendir(df, strateji, gecmis_dahil, takvim_dahil):
...
def main():
    ...
    print("=" * 100)
    print("TAKVİM ÖZNİTELİK ABLASYONU (geçmiş YOK sabit; appointment_year, _month,")
    print("_day_of_week, _hour çıkarılıyor - lead_time HARİÇ)")
    print("=" * 100)

    sonuc = []
    for skod, sad in stratejiler:
        for takvim_dahil, tetiket in [(True, "Takvim VAR"), (False, "Takvim YOK")]:
            m = degerlendir(df, skod, gecmis_dahil=False, takvim_dahil=takvim_dahil)
            sonuc.append({"Bölme": sad, "Takvim": tetiket, **m})
            print(f"{sad:46s} | {tetiket:11s} | "
    ...
    genis = []
    for skod, sad in stratejiler:
        var_row = [s for s in sonuc if s["Bölme"] == sad and s["Takvim"] == "Takvim VAR"][0]
        yok_row = [s for s in sonuc if s["Bölme"] == sad and s["Takvim"] == "Takvim YOK"][0]
        fark = yok_row["ROC-AUC"] - var_row["ROC-AUC"]
        genis.append({
    ...
    df_genis = pd.DataFrame(genis)

    print("\n" + "=" * 100)
    print("KARŞILAŞTIRMA TABLOSU (geçmiş YOK sabit)")
    print("=" * 100)
    print(df_genis.to_string(index=False))

    kron = df_genis[df_genis["Bölme Stratejisi"] == stratejiler[2][1]].iloc[0]
    print("\n" + "-" * 100)
    print("KRONOLOJİK SENARYO (asıl soru):")
    print(f"  Takvim VAR: {kron['ROC-AUC (Takvim VAR)']:.4f}")
    print(f"  Takvim YOK: {kron['ROC-AUC (Takvim YOK)']:.4f}")
    print(f"  Fark (YOK - VAR): {kron['Fark (YOK - VAR)']:+.4f}  -> {kron['Yön']}")

    uzun_yolu = KOK / "veriler" / "takvim_ablasyon_uzun.csv"
    genis_yolu = KOK / "veriler" / "takvim_ablasyon_karsilastirma.csv"
