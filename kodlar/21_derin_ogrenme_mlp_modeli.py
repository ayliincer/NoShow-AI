"""
21_derin_ogrenme_mlp_modeli.py

AMAÇ (Hakem Eleştirisi Giderme - Eksiklik #2):
Model havuzunda yalnızca klasik/ağaç tabanlı algoritmaların bulunması, hakemlerin
"Neden Derin Öğrenme (TabNet / MLP) denenmedi?" eleştirisine yol açmaktaydı.

Bu script, Çok Katmanlı Perseptron (Multi-Layer Perceptron / MLP) modelini:
  1) 17_model_pipeline_ve_capraz_dogrulama.py ile BİREBİR AYNI 5 katlı çapraz
     doğrulama protokolüyle (StratifiedKFold, aynı metrikler, aynı random_state)
     eğitim seti üzerinde test eder ve sonucu model_karsilastirma_sonuclari.csv'ye ekler.
  2) 18_dis_dogrulama_ve_model_secimi.py ile BİREBİR AYNI saklı dış test protokolüyle
     (N=9.329) değerlendirir ve sonucu nihai_dis_dogrulama_sonuclari.csv'ye ekler.

Böylece MLP, mevcut 6 modelle (LR, DT, RF, XGBoost, LightGBM, CatBoost) tam olarak
aynı koşullar altında, sızıntısız (leakage-free) biçimde karşılaştırılabilir hale gelir.
"""
...
def mlp_pipeline_olustur() -> ImbPipeline:
    # MLP; StandardScaler (gradyan tabanlı optimizasyon için ölçekleme şart) +
    # SMOTE (sınıf dengesizliğini eğitim katlarında sentetik örnekle giderme,
    # yalnızca eğitim katına uygulanır -> sızıntı yok) + MLPClassifier.
    return ImbPipeline([
...
def csv_ye_ekle_veya_guncelle(csv_yolu: Path, yeni_satir: dict, sirala_sutun: str):
    ...
    mevcut = mevcut[mevcut["Model"] != yeni_satir["Model"]]  # tekrar koşumlarda üzerine yazma
...
def main():
    ...
    # --- 1) Çapraz Doğrulama (Eğitim Seti) ---
    ...
    # --- 2) Saklı Dış Test Seti Doğrulaması ---
