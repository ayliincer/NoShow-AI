"""
24_esleştirilmiş_simulasyon_ve_anlamlilik_testi.py

AMAÇ (Hakem Eleştirisi Giderme - Risk #7):
Önceki simülasyonda (22. script) Politika A ve Politika B, FARKLI rastgele
tohumlarla (RASTGELE_TOHUM ve RASTGELE_TOHUM+1) simüle edilmiş; yani her
"gün", iki politika için de FARKLI bir hasta popülasyonuyla karşılaştırılmıştı
(eşleştirilmemiş/unpaired tasarım). Gün-içi varyans (~12 dk SD) gerçek etkiden
(~1 dk fark) çok daha büyük olduğu için, bu tasarımla hesaplanan p-değerleri
"Ort. Hasta Bekleme Süresi" ve "Hekim Mesai Aşımı" için anlamsız (p>0.05) çıkar.

Bu script, Deneysel Tasarım literatüründe standart olan ORTAK RASTGELE SAYILAR
(Common Random Numbers / CRN) tekniğini uygular: HER GÜN için TEK BİR hasta
popülasyonu (aynı hastalar, aynı gerçek gelme/gelmeme durumları) üretilir ve
HER İKİ POLİTİKA DA bu AYNI popülasyona karşı simüle edilir. Böylece gün-içi
gürültü ortadan kalkar, yalnızca politika farkı ölçülür ve GÜN BAZINDA
EŞLEŞTİRİLMİŞ (paired) bir karşılaştırma / istatistiksel test mümkün olur.

SIZINTI KORUMASI: Olasılık dağılımı yine yalnızca saklı dış test setinden
(N=9.329) gelir; model/veri protokolünde bir değişiklik yoktur.
"""
...
def sampiyon_modelin_gercek_olasiliklarini_yukle(kok: Path) -> np.ndarray:
    paket = joblib.load(kok / "modeller" / "nihai_no_show_model_paketi.joblib")
    ...
    X_test = test.drop(columns=["no_show"])[paket["sutun_siralamasi"]]
...
def esleştirilmiş_simulasyonu_yurut(tum_olasiliklar: np.ndarray) -> pd.DataFrame:
    ...
    for gun in range(GUNLUK_SIMULASYON_TEKRARI):
        ...
        muayene_tohumu = int(populasyon_rng.integers(0, 1_000_000))  # her gün, her iki politika için AYNI
...
def anlamlilik_testleri_uygula(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for kod, ad in metrikler.items():
        a = statik[kod].values
        b = hibrit[kod].values
        fark = b - a  # Hibrit - Statik (paired difference)
        t_stat, t_p = stats.ttest_rel(a, b)
        try:
            w_stat, w_p = stats.wilcoxon(a, b)
        except ValueError:
            w_stat, w_p = np.nan, np.nan
        cohen_d_z = fark.mean() / fark.std(ddof=1)  # paired Cohen's d_z
