...
def veri_setlerini_yukle(egitim_yolu: Path, test_yolu: Path) -> tuple:
    """
    Önceki adımdan elde edilen eğitim ve test veri setlerini ham haliyle yükler.
    """
    try:
...
def kardinalite_profilini_raporla(veri: pd.DataFrame, haric_tutulacaklar: list) -> list:
    """
    Veri setindeki object tipindeki sütunların benzersiz sınıf (kardinalite) 
    yoğunluklarını hesaplar ve SCI standardında akademik tablo olarak basar.
    Yüksek kardinaliteli değişkenleri tespit ederek metodolojik uyarı döndürür.
    """
    print("=" * 110)
    ...
    # Sadece object tipindeki sözel sütunlar taranır
...
def kategorik_kodlama_uygula(egitim: pd.DataFrame, test: pd.DataFrame, profil_listesi: list, ham_zaman_sutunlari: list) -> tuple:
    ...
    Uygun bulunan kategorik değişkenlere One-Hot Encoding uygular.
    Parametreleri yalnızca eğitim verisinden fit eder, teste sadece transform uygular.
    İşlem sonunda align() ile sütun yapılarını eşitler ve doğrular.
    """
    # 1. Ham zaman sütunlarının veri setinden uzaklaştırılması
    print("\n" + "-" * 90)
    ...
    # Yeni ve eski sklearn sürümleri arasındaki uyumluluk sparse_output parametresiyle güvenli yönetilir
    try:
    ...
    # Encoder yalnızca eğitim verisi üzerinde eğitilir (Fit)
    kodlayici.fit(egitim[one_hot_sutunlari])

    # Yeni sütun isimleri güvenli biçimde alınır
    yeni_sutun_isimleri = kodlayici.get_feature_names_out(one_hot_sutunlari)

    # Eğitim ve test verileri dönüştürülür (Transform)
    ...
    # Orijinal metin sütunları düşürülür ve yeni dummy sütunlar eklenir
    ...
    # 3. Hizalama (Alignment) Aşaması
    # Eğitim ve test setlerinin sütun yapılarının ve sıralarının birebir aynı olması tescil edilir
...
def verileri_diske_kaydet(egitim: pd.DataFrame, test: pd.DataFrame, egitim_yolu: Path, test_yolu: Path):
    """
    Kategorik kodlama işlemleri tamamlanan nihai veri setlerini diske kaydeder.
    """
